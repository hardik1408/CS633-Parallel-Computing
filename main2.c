...
int main(int argc, char** argv) {
    ...
    // Print debug information for each process
    ...
    if (rank == 0) {
        printf("Process 0: Starting data distribution\n");
        for (int r = 0; r < size; ++r) {
            int px_ = r % PX;
            int py_ = (r / PX) % PY;
            int pz_ = r / (PX * PY);

            int lx_ = NX / PX + (px_ < NX % PX);
            int ly_ = NY / PY + (py_ < NY % PY);
            int lz_ = NZ / PZ + (pz_ < NZ % PZ);

            int ox = (NX / PX) * px_ + (px_ < NX % PX ? px_ : NX % PX);
            int oy = (NY / PY) * py_ + (py_ < NY % PY ? py_ : NY % PY);
            int oz = (NZ / PZ) * pz_ + (pz_ < NZ % PZ ? pz_ : NZ % PZ);

            float* temp = (float*)malloc(sizeof(float) * lx_ * ly_ * lz_ * NC);
            for (int z = 0; z < lz_; ++z)
                for (int y = 0; y < ly_; ++y)
                    for (int x = 0; x < lx_; ++x)
                        for (int t = 0; t < NC; ++t)
                            temp[(((z * ly_ + y) * lx_ + x) * NC) + t] =
                                full_data[(((oz + z) * NY + (oy + y)) * NX + (ox + x)) * NC + t];

            if (r == 0) local_data = temp;
            else {
                printf("Process 0: Sending data to process %d\n", r);
                MPI_Send(temp, lx_ * ly_ * lz_ * NC, MPI_FLOAT, r, 0, MPI_COMM_WORLD);
            }
            if (r != 0) free(temp);
        }
        printf("Process 0: Data distribution complete\n");
    } else {
        printf("Process %d: Receiving data from process 0\n", rank);
        MPI_Recv(local_data, local_size * NC, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
