...
int main(int argc, char **argv) {
    ...
    // Calculate Cartesian coordinates directly from rank
    int px = rank % PX;
    int py = (rank / PX) % PY;
    int pz = rank / (PX * PY);
    ...
    for (int t = 0; t < NC; t++) {
        if(rank == 0) {
            for (int p = 0; p < size; p++) {
                // Calculate process coordinates directly from rank
                int p_px = p % PX;
                int p_py = (p / PX) % PY;
                int p_pz = p / (PX * PY);
        ...
        for (int dz = -1; dz <= 1; dz++) {
            for (int dy = -1; dy <= 1; dy++) {
                for (int dx = -1; dx <= 1; dx++) {
                    ...
                    // Calculate neighbor coordinates and check if they exist
                    int nbr_px = px + dx;
                    int nbr_py = py + dy;
                    int nbr_pz = pz + dz;
                    
                    if(nbr_px < 0 || nbr_px >= PX ||
                       nbr_py < 0 || nbr_py >= PY ||
                       nbr_pz < 0 || nbr_pz >= PZ) {
                        continue; // at global boundary, no exchange
                    }
                    
                    // Calculate neighbor rank directly
                    int nbr = nbr_px + nbr_py * PX + nbr_pz * PX * PY;
