...
int main(int argc, char **argv) {
    ...
    for (int t = 0; t < NC; t++) {
        MPI_Offset offset = (MPI_Offset)(t * total_points + offset_z * NX * NY + offset_y * NX + offset_x) * sizeof(float);
        float *time_slice = local_data + t * local_size;
        for (int z = 0; z < lz; z++) {
            for (int y = 0; y < ly; y++) {
                MPI_Offset slice_offset = offset + (z * NX * NY + y * NX) * sizeof(float);
                MPI_File_read_at(fh, slice_offset, time_slice + IDX(0, y, z, lx, ly), lx, MPI_FLOAT, &status);
            }
        }
    }
    MPI_File_close(&fh);
    ...
    // Extended buffer for halo exchange
    int ext_nx = lx + 2, ext_ny = ly + 2, ext_nz = lz + 2;
    int ext_size = ext_nx * ext_ny * ext_nz;
    float *ext_data = (float*)calloc(ext_size * NC, sizeof(float));

    // Copy local_data to ext_data interior
    for (int t = 0; t < NC; t++) {
        for (int z = 0; z < lz; z++) {
            for (int y = 0; y < ly; y++) {
                for (int x = 0; x < lx; x++) {
    ...
    // Halo exchange (unchanged from original)
    for (int t = 0; t < NC; t++) {
        for (int dz = -1; dz <= 1; dz++) {
            for (int dy = -1; dy <= 1; dy++) {
                for (int dx = -1; dx <= 1; dx++) {
                    ...
                    if (nbr_coords[0] < 0 || nbr_coords[0] >= PX ||
                        nbr_coords[1] < 0 || nbr_coords[1] >= PY ||
                        nbr_coords[2] < 0 || nbr_coords[2] >= PZ) continue;
                    int nbr;
                    ...
                    float *sendbuf = (float*)malloc(sizeof(float) * block_size);
                    float *recvbuf = (float*)malloc(sizeof(float) * block_size);
                    ...
                    for (int z = send_z_start; z <= send_z_end; z++) {
                        for (int y = send_y_start; y <= send_y_end; y++) {
                            for (int x = send_x_start; x <= send_x_end; x++) {
                                ...
                                sendbuf[idx] = ext_data[ext_idx * NC + t];
                                idx++;
                            }
                        }
                    }

                    MPI_Sendrecv(sendbuf, block_size, MPI_FLOAT, nbr, 100,
                                 recvbuf, block_size, MPI_FLOAT, nbr, 100,
                    ...
                    for (int z = 0; z < block_nz; z++) {
                        for (int y = 0; y < block_ny; y++) {
                            for (int x = 0; x < block_nx; x++) {
                                ...
                                ext_data[ext_idx * NC + t] = recvbuf[idx];
                                idx++;
                            }
                        }
                    }
        ...
        for (int z = 1; z <= lz; z++) {
            for (int y = 1; y <= ly; y++) {
                for (int x = 1; x <= lx; x++) {
                    ...
                    for (int i = 0; i < 6; i++) {
                        int nb_global_x = global_x + d[i][0];
                        int nb_global_y = global_y + d[i][1];
                        int nb_global_z = global_z + d[i][2];
                        if (nb_global_x < 0 || nb_global_x >= NX ||
                            nb_global_y < 0 || nb_global_y >= NY ||
                            nb_global_z < 0 || nb_global_z >= NZ) continue;
                        int nb_ext_idx = IDX(x + d[i][0], y + d[i][1], z + d[i][2], ext_nx, ext_ny);
    ...
    // Compute and reduce timings
    double local_read_time = t2 - t1;
    double local_main_time = t3 - t2;
    double local_total_time = t4 - t1;

    double max_read_time, max_main_time, max_total_time;
    MPI_Reduce(&local_read_time, &max_read_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce(&local_main_time, &max_main_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
