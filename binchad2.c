...
int main(int argc, char **argv) {
    ...
    /*------------------------------------------------------------------
      ...
      contains NC floating-point numbers stored as 8-byte (double precision)
      values without delimiters. We use an MPI derived datatype to create a
      noncontiguous view of the file, perform a collective read, and then
    ...
    /* Create an indexed datatype for the file view.
       Each point's data occupies NC doubles, starting at the offset:
         global_index * NC * sizeof(double)
    */
    ...
    for (int i = 0; i < local_line_count; i++) {
        displacements[i] = (MPI_Aint)local_line_indices[i] * NC * sizeof(double);
        blocklens[i] = NC;
    }
    MPI_Datatype filetype;
    MPI_Type_create_hindexed(local_line_count, blocklens, displacements, MPI_DOUBLE, &filetype);
    MPI_Type_commit(&filetype);
    MPI_File_set_view(fh, 0, MPI_DOUBLE, filetype, "native", MPI_INFO_NULL);

    /* Read the binary data collectively.
       Allocate a temporary buffer to store the data as doubles.
    */
    double *temp_buffer = (double *)malloc(local_line_count * NC * sizeof(double));
    MPI_File_read_all(fh, temp_buffer, local_line_count * NC, MPI_DOUBLE, MPI_STATUS_IGNORE);
    MPI_File_close(&fh);
    MPI_Type_free(&filetype);
    free(displacements);
    free(blocklens);

    /* Convert the data from double precision to float.
       This conversion is necessary since the computation code expects floats.
    */
    float *local_data = (float *)malloc(local_line_count * NC * sizeof(float));
    for (int i = 0; i < local_line_count * NC; i++) {
        local_data[i] = (float)temp_buffer[i];
    }
    free(temp_buffer);
